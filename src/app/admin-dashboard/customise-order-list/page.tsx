...
export default function OrderListPage() {
  ...
  const handleToggleList = () => {
    if (!apiCalled) {
        setLoading(true);
      axios.get("/api/customise-order-list-for-admin").then((res) => {
        if (res.data.success) {
  ...
  try {
    const response = await fetch('/api/update-customise-desposition', {
      method: 'PUT',
      headers: {
...
const handleUpdateDeliveryDate = async () => {
  ...
  try {
    const response = await axios.post("/api/update-customise-delivery-date", {
  ...
  <Stack spacing={2}>
    {filteredOrders.map((order) => (
      <Box key={order._id} p={2} border="1px solid #ccc" borderRadius={2}>
        ...
        <Typography><strong>Quantity:</strong> {order.product[0].quantity}</Typography>
        ...
        <Table>
            ...
            {filteredOrders.map((order) => (
                ...
                <TableCell>{order.product[0].quantity}</TableCell>
            ...
            <DialogContent>
              ...
              {dialogType === "product" && (
                    <>
                        ...
                        {dialogData?.product?.map((item, index) => (
                        <div key={index} style={{ marginBottom: 16 }}>
                            ...
                            <Tooltip title="Copy URL">
                                <IconButton onClick={() => handleCopy(item?.url?.url)}>
                                <ContentCopyIcon />
                                </IconButton>
                            </Tooltip>
