...
import GroupAddIcon from '@mui/icons-material/GroupAdd'; // For View Referred User
import MonetizationOnIcon from '@mui/icons-material/MonetizationOn'; // For Toggle Referred Payment Status
...
export default function OrderListPage() {
  ...
  const [referredPaymentDialogOpen, setReferredPaymentDialogOpen] = useState(false);
  const [referredPayStatus, setReferredPayStatus] = useState("Not Paid"); // or "Paid"
  ...
  const handleToggleList = () => {
    if (!apiCalled) {
        setLoading(true);
      axios.get("/api/referrel-list-api").then((res) => {
        if (res.data.success) {
...
const openReferredPaymentDialog = (order:any) => {
  setSelectedOrder(order);
  setReferredPayStatus(order.refferel_payment_status ? "Paid" : "Not Paid");
  setReferredPaymentDialogOpen(true);
};

const handleUpdateReferredPayment = async () => {
  if (!selectedOrder) {
  ...
  try {
    const response = await fetch("/api/update-referred-payment", {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify({
        id: selectedOrder._id,
        referred_paid: referredPayStatus
      }),
    });
    ...
    if (response.ok) {
      ...
      const updatedOrders = filteredOrders.map((order) =>
        order._id === selectedOrder._id
          ? { ...order, refferel_payment_status: referredPayStatus }
          : order
      );
      ...
      setSnackbar({
        open: true,
        message: 'Reffer payment Status updated successfully!',
        severity: 'success',
      });
      setReferredPaymentDialogOpen(false);
    } else {
      setSnackbar({
        open: false,
      ...
      <Box key={order._id} p={2} border="1px solid #ccc" borderRadius={2}>
        ...
        <Box mt={1}>
          <Tooltip title="View Address">
            ...
            <Tooltip title="View Referred User">
              <IconButton onClick={() => handleOpenDialog("referredUser", order)}>
                <GroupAddIcon sx={{ color: "#8e24aa" }} />
              </IconButton>
            </Tooltip>

           <Tooltip title="Toggle Referred Payment Status">
              <IconButton onClick={() => openReferredPaymentDialog(order)}>
                <MonetizationOnIcon sx={{ color: order.refferel_payment_status ? "green" : "red" }} />
              </IconButton>
            </Tooltip>
        ...
        <Table>
            ...
            {filteredOrders.map((order) => (
                ...
                <TableCell>
                    ...
                    {/* New: View Referred User */}
                    <Tooltip title="View Referred User">
                      <IconButton onClick={() => handleOpenDialog("referredUser", order)}>
                        <GroupAddIcon sx={{ color: "#8e24aa" }} /> {/* Purple color */}
                      </IconButton>
                    </Tooltip>

                    {/* New: Toggle Referred Amount Pay Status */}
                    <Tooltip title="Toggle Referred Payment Status">
                        <IconButton onClick={() => openReferredPaymentDialog(order)}>
                          <MonetizationOnIcon sx={{ color: order.refferel_payment_status ? "green" : "red" }} />
                        </IconButton>
          ...
          <Dialog open={!!dialogType} onClose={handleCloseDialog} maxWidth="sm" fullWidth>
            ...
            <DialogContent>
              ...
              {dialogType === "referredUser" && (() => {
                const u = dialogData?.referred_users_details?.[0];
                return (
                  <>
                    <Typography variant="h6">{u?.name}</Typography>
                    <Typography>Email: {u?.email}</Typography>
          ...
          <Dialog open={despositionDialogOpen} onClose={() => setDespositionDialogOpen(false)}>
            ...
            <Dialog open={referredPaymentDialogOpen} onClose={() => setReferredPaymentDialogOpen(false)}>
                <DialogTitle>Update Referred Payment Status</DialogTitle>
                <DialogContent>
                  <FormControl fullWidth sx={{ mt: 2 }}>
                    <InputLabel>Payment Status</InputLabel>
                    <Select
                      value={referredPayStatus}
                      label="Payment Status"
                      onChange={(e) => setReferredPayStatus(e.target.value)}
                    >
                ...
                <DialogActions>
                  <Button onClick={() => setReferredPaymentDialogOpen(false)}>Cancel</Button>
                  <Button variant="contained" color="primary" onClick={handleUpdateReferredPayment}>
                    Update
                  </Button>
                </DialogActions>
              </Dialog>
